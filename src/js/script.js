document.addEventListener('DOMContentLoaded', function() {
    ...
    async function fetchNews() {
        try {
            const response = await fetch('/spacediscovery/src/js/news.json'); // Assuming your JSON file is named news.json
            if (!response.ok) {
    ...
    function displayRecentArticles(articles) {
        ...
        recentArticles.forEach(article => {
            ...
            card.addEventListener('click', () => {
                ...
                window.open(`/spacediscovery/src/articles/article.html?id=${article.id}`, '_blank');
            });
